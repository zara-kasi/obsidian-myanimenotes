        ...
        try {
            ...
            if (response.status === 429) {
                if (attempt < MAX_RETRIES) {
                    ...
                    debug.log(
                        `[MAL API] Rate limit hit (429). Retrying in ${delay}ms (attempt ${
                            attempt + 1
                        }/${MAX_RETRIES})...`
                    );
                    await sleep(delay);
            ...
            if (attempt < MAX_RETRIES) {
                ...
                debug.log(
                    `[MAL API] Request failed: ${errorMessage}. Retrying in ${delay}ms (attempt ${
                        attempt + 1
                    }/${MAX_RETRIES})...`
                );
                await sleep(delay);
...
async function fetchAllPages(
    ...
    do {
        ...
        if (data.data && Array.isArray(data.data)) {
            ...
            if (offset === 0 && data.data.length > 0) {
                debug.log("[MAL API] Sample response structure:", {
                    ...
                    listStatusKeys: data.data[0].list_status
                        ? Object.keys(data.data[0].list_status)
                        : [],
        ...
        if (offset > 10000) {
            debug.log(
                "[MAL API] Safety limit reached (10000 items), stopping pagination"
            );
            break;
        }
    } while (nextUrl);
    ...
    debug.log(
        `[MAL API] Fetched total of ${allItems.length} items from ${endpoint}`
    );
    return allItems;
}
